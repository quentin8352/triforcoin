...
#if defined(NDEBUG)
# error "Triforcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xf41de32fc23b18fee9974cedecc3d0394034834e4d9ed0fd2c3cf9abf0148561");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Triforcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Triforcoin Signed Message:\n";
...
{
    // Triforcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Triforcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Triforcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Triforcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Triforcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Triforcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5f4b84a5bb9a680736c29cf13b4caf19f1eba724c78aa44d2ece9b654a4bdcc6");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "TriForMine is the best coder of the world, he will be the best game developer";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("049fe51f3d5c59bfb692c2451538788fb403f476a1e074e5e76000d62e9f3b44bd530d0ea9e6feb4d2f7a1d8c41f1c335218903f2b08bf7ada258010640811242e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1527440976;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 386859513;

        if (fTestNet)
        {
            block.nTime    = 1527440957;
            block.nNonce   = 385443583;
        }
	...
	if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x7a55d336ebf24dd25c98a68044d958ff8ec034216d84f41514ed51dd143b1b3b"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Triforcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TriforcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TriforcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TriforcoinMiner(CWallet *pwallet)
{
    printf("TriforcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("triforcoin-miner");
    ...
    try { loop {
        ...
        printf("Running TriforcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TriforcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TriforcoinMiner, pwallet));
}
